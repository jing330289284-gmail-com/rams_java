			...
			if (methodName.equals("toroku")) {
				...
				if (UtilsCheckMethod.isNullOrEmpty(p.getAdmissionStartDate())) {
					errors.rejectValue("admissionStartDate", "",
							StatusCodeToMsgMap.getErrMsgbyCodeReplace("MSG001", "入場年月日"));
				}
				// 客户不能为空
				...
				if (!p.getWorkState().equals("0")) {
					if (UtilsCheckMethod.isNullOrEmpty(p.getAdmissionEndDate())) {
						errors.rejectValue("unitPrice", "",
								StatusCodeToMsgMap.getErrMsgbyCodeReplace("MSG001", "退場年月日"));
					}
				}
				// 精算范围后面小于前面
				if (!UtilsCheckMethod.isNullOrEmpty(p.getPayOffRange1())
					...
					if (Integer.parseInt(p.getPayOffRange1()) > Integer.parseInt(p.getPayOffRange2())) {
						errors.rejectValue("payOffRange1", "",
								StatusCodeToMsgMap.getErrMsgbyCodeReplace("MSG009", "精算"));
					}
				}
				// 入场时间大于退场时间
				if (!UtilsCheckMethod.isNullOrEmpty(p.getAdmissionEndDate())
